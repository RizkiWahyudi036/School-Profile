...
<?php
            ...
            include('koneksi.php');
            $nisn = $_GET['nisn'];
            $show = mysqli_query($connection, "SELECT * FROM db_siswa WHERE nisn = '$nisn'");
                ...
                <div class="col-md-8">
                    <div class="edit-form h-100">
                        <div class="row justify-content-center align-items-center">
                            <div class="col-7">
                                <div class="edit-data">
                                    <form action="aksi_edit_siswa.php" method="post">
                                        <label for="nisn" class="form-label" >NISN</label>
                                        <input type="text" name="nisn" class="form-control" value="<?php echo $d['nisn']; ?>" required />
                                        <label for="nama" class="form-label" >Nama</label>
                                        <input type="text" name="nama" class="form-control" value="<?php echo $d['nama']; ?>" required />
                                        <label for="kelas" class="form-label">Kelas</label>
                                        <input type="text" name="kelas" class="form-control" value="<?php echo $d['kelas']; ?>" required />
                                        <label for="jk" class="form-label">Jenis Kelamin</label>
                                        <select name="jk" class="form-select" aria-label="Default select example"
                                            required>
                                            <option value="">Pilih</option>
                                            <option value="Pria" <?php if($d['jk'] == 'Pria'){echo 'selected';} ?>> Pria</option>
                                            <option value="Wanita" <?php if($d['jk'] == 'Wanita'){echo 'selected';} ?>> Wanita</option>
                                        </select>
                                        <label for="alamat" class="form-label">Alamat</label>
                                        <input type="text" name="alamat" class="form-control" value="<?php echo $d['alamat']; ?>" required />
                                        <p></p>
                                        <input type="submit" name="edit" value="Edit" class="back" style="background-color:rgb(0, 153, 255); color:white">
                                    </form>
